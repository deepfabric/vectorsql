...
func (r *rule) Rewrite(e extend.Extend) (filter.Filter, filter.Filter, error) {
	bs, qs, err := r.disintegration(e, []bm.Bm{}, []string{})
	if err != nil {
		return nil, nil, err
	}
...
func (r *rule) disintegration(e extend.Extend, bs []bm.Bm, qs []string) ([]bm.Bm, []string, error) {
	switch v := e.(type) {
	case *value.Bool:
		query, err := r.genResult(v)
		if err != nil {
			return nil, nil, err
		}
		qs = append(qs, query)
	...
	case *extend.ParenExtend:
		var err error
		var bt []bm.Bm
		if bt, qs, err = r.disintegration(v.E, []bm.Bm{}, qs); err != nil {
			return nil, nil, err
		}
		bs = append(bs, bm.Bm{Bs: bt})
	case *extend.BinaryExtend:
		var err error
		if bs, qs, err = r.disintegrationBinary(v, bs, qs); err != nil {
			return nil, nil, err
		}
	}
	return bs, qs, nil
...
func (r *rule) disintegrationBinary(e *extend.BinaryExtend, bs []bm.Bm, qs []string) ([]bm.Bm, []string, error) {
	switch e.Op {
	case overload.EQ, overload.LT, overload.GT, overload.LE,
		overload.GE, overload.NE, overload.Like, overload.NotLike:
		query, err := r.genResult(e)
		if err != nil {
			return nil, nil, err
		}
		qs = append(qs, query)
	...
	case overload.Or:
		var err error
		if bs, qs, err = r.disintegration(e.Left, bs, qs); err != nil {
			return nil, nil, err
		}
		bs[len(bs)-1].IsOr = true
		if bs, qs, err = r.disintegration(e.Right, bs, qs); err != nil {
			return nil, nil, err
		}
		return bs, qs, nil
	case overload.And:
...
func (r *rule) genResult(e extend.Extend) (string, error) {
	...
	switch len(ts) {
	case 0:
		return fmt.Sprintf("(SELECT groupBitmapState(seq) FROM people WHERE %s) AS bm%v", e, r.cnt), nil
	case 1:
		return fmt.Sprintf("(SELECT groupBitmapState(seq) FROM %s WHERE %s) AS bm%v", ts[0], e, r.cnt), nil
	}
